...
import { Clock, MapPin, Users, Tag, Phone, Mail, Calendar, AlertCircle } from "lucide-react";
...
export const DonationCard: React.FC<DonationCardProps> = ({ donation }) => {
  return (
    <div className="bg-[var(--color-surface)] rounded-tr-2xl rounded-tl-2xl border border-[var(--color-border)] shadow-[var(--shadow-card)] overflow-hidden transition-all duration-200 hover:shadow-lg hover:border-[var(--color-primary)]/20">
      ...
      <div className="p-4 pt-0">
        ...
        <div className="flex items-center gap-2 mb-4">
          <div className="p-1.5 rounded-lg bg-[var(--color-accent-blue)]/10">
            {donation.contactMethod === "email" && <Mail className="h-3.5 w-3.5 text-[var(--color-accent-blue)]" />}
            {donation.contactMethod === "phone" && <Phone className="h-3.5 w-3.5 text-[var(--color-accent-blue)]" />}
            {donation.contactMethod === "both" && <Mail className="h-3.5 w-3.5 text-[var(--color-accent-blue)]" />}
          </div>
          <div>
        ...
        {donation.dietaryInfo && donation.dietaryInfo.length > 0 && (
          <div className="mb-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs text-[var(--color-text-muted)]">Dietary Info</p>
            </div>
        ...
        {donation.specialInstructions && (
          <div className="mb-4 p-3 bg-[var(--color-background)] rounded-lg border border-[var(--color-border)]">
            <p className="text-xs text-[var(--color-text-muted)] mb-1">Special Instructions</p>
            <p className="text-sm text-[var(--color-text-primary)]">{donation.specialInstructions}</p>
          </div>
        )}
